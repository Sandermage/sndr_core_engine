...
P67B_NEW = (
    ...
    "                        # Fix: per-K1 SimpleNamespace holder on `self` isolates\n"
    "                        # the grow-check (lines below). mid_o/lse allocation is\n"
    "                        # handled by WorkspaceManager.get_simultaneous() inside\n"
    "                        # triton_turboquant_decode_attention (PR #40798 removed\n"
    "                        # buf_holder from that API; dev270+ uses WorkspaceManager).\n"
    "                        # PN33 ensures warmup covers K+1 batch sizes so\n"
    "                        # WorkspaceManager is correctly pre-sized at lock time.\n"
    ...
    # buf_holder removed: upstream PR #40798 dropped it in dev270 — WorkspaceManager handles allocation.
    ...
    "                'P67b dispatch failed (%s), attempting fallthrough guard',\n"
    "                _genesis_p67b_err, exc_info=True,\n"
    "            )\n"
    "            # ── [Genesis P67b fallthrough guard v7.73] ─────────────────\n"
    "            # K+1 spec-verify batches must NOT hit the decode path's\n"
    "            # layer-cached buffers (sized for max_num_seqs, but K+1\n"
    "            # needs B*K1 rows). Create a throwaway per-call holder so\n"
    "            # the kernel self-allocates correct-sized buffers.\n"
    "            try:\n"
    "                from vllm.v1.attention.ops.triton_turboquant_decode import (\n"
    "                    triton_turboquant_decode_attention as _fg_decode,\n"
    "                )\n"
    "                import torch as _fg_tc\n"
    "                _fg_Hk = key.shape[1]\n"
    "                _fg_hpk = self.num_heads // _fg_Hk if _fg_Hk else 0\n"
    "                _fg_shape_ok = (self.num_heads >= 8\n"
    "                    and self.head_size in (128, 256) and _fg_hpk >= 2)\n"
    "                _fg_k1 = attn_metadata.max_query_len\n"
    "                _fg_b = N // _fg_k1\n"
    "                if (_fg_shape_ok and 1 < _fg_k1 <= 16 and N > 0\n"
    "                        and (N % _fg_k1) == 0\n"
    "                        and attn_metadata.query_start_loc is not None\n"
    "                        and attn_metadata.query_start_loc.shape[0] == _fg_b + 1):\n"
    "                    _fg_q = q[:N].view(N, self.num_heads, self.head_size)\n"
    "                    _fg_offs = _fg_tc.arange(_fg_k1, device=q.device,\n"
    "                        dtype=attn_metadata.seq_lens.dtype)\n"
    "                    _fg_sl = (attn_metadata.seq_lens[:_fg_b, None]\n"
    "                        - _fg_k1 + 1 + _fg_offs[None, :]).reshape(-1)\n"
    "                    _fg_bt = attn_metadata.block_table[:_fg_b].repeat_interleave(\n"
    "                        _fg_k1, dim=0)\n"
    "                    _fg_out_buf = _fg_tc.empty(\n"
    "                        (N, self.num_heads, self.head_size),\n"
    "                        dtype=q.dtype, device=q.device)\n"
    "                    _fg_o = _fg_decode(query=_fg_q, kv_cache=kv_cache,\n"
    "                        block_table=_fg_bt, seq_lens=_fg_sl,\n"
    "                        Pi=Pi, centroids=centroids, scale=self.scale,\n"
    "                        mse_bits=self.tq_config.key_mse_bits,\n"
    "                        key_packed_size=self.tq_config.key_packed_size,\n"
    "                        value_quant_bits=self.tq_config.effective_value_quant_bits,\n"
    "                        key_fp8=self.tq_config.key_fp8,\n"
    "                        norm_correction=self.tq_config.norm_correction,\n"
    "                        PiT=PiT, output_buf=_fg_out_buf,\n"
    "                        mid_o_buf=None, lse_buf=None,\n"
    "                    ).view(N, self.num_heads, self.head_size)\n"
    "                    if output.ndim == 3:\n"
    "                        output[:N] = _fg_o\n"
    "                    else:\n"
    "                        output[:N] = _fg_o.reshape(N, -1)\n"
    "                    _genesis_p67b_logging.getLogger('genesis.wiring.p67b').warning(\n"
    "                        'P67b fallthrough guard succeeded (B=%d, K1=%d)',\n"
    "                        _fg_b, _fg_k1)\n"
    "                    return output\n"
    "            except Exception:\n"
    "                _genesis_p67b_logging.getLogger('genesis.wiring.p67b').exception(\n"
    "                    'P67b fallthrough guard failed')\n"
    "\n"
    "        if not attn_metadata.is_prefill:\n"
)
