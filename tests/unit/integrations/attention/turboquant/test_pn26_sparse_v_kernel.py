...
backend. Run via:
    pytest tests/legacy/test_pn26_sparse_v_kernel.py -v
"""
...
def _torch_cuda_available() -> bool:
    """Safe torch+CUDA detection. Returns False if torch absent (does NOT
    raise ModuleNotFoundError at collection time — required for torch-less
    CI gate to collect this file's CPU-only threshold tests).
    """
    try:
...
requires_cuda = pytest.mark.skipif(
    not _torch_cuda_available() or not _has_vllm_triton_utils(),
