...
def _install_fake_rejection_sampler(received, monkeypatch):
    ...
    All sys.modules entries go through monkeypatch.setitem so they are
    restored on teardown — a bare setdefault("vllm", ...) leaks a spec-less
    `vllm` module that later breaks importlib.util.find_spec("vllm") and
    pollutes other suites (e.g. the environment report tests)."""
    ...
    monkeypatch.setitem(sys.modules, "vllm.v1.sample.rejection_sampler", mod)
    monkeypatch.setattr(
        sys.modules["vllm.v1.sample"], "rejection_sampler", mod, raising=False
    )
    return mod
...
def test_pn282_forwards_future_kwargs_transparently(monkeypatch):
    received = {}
    mod = _install_fake_rejection_sampler(received, monkeypatch)
    rec = {}
...
def test_pn248_forwards_future_kwargs_transparently(monkeypatch, tmp_path):
    ...
    received = {}
    mod = _install_fake_rejection_sampler(received, monkeypatch)
    # PN248 is opt-in (default OFF); enable it so apply() actually wraps
    # rejection_sample instead of short-circuiting with "skipped".
    monkeypatch.setenv("GENESIS_ENABLE_PN248_ACCEPTANCE_TRACE", "1")
