...
def mock_tq_impl_module(monkeypatch):
    ...
    # Inject via monkeypatch.setitem so the fakes are restored on teardown;
    # a bare sys.modules[...] = ModuleType(...) leaks a spec-less `vllm`
    # module that breaks a later importlib.util.find_spec("vllm") and
    # pollutes other suites.
...
def mock_grouped_router_module(monkeypatch):
    ...
    # Parent path. Inject via monkeypatch.setitem so the fakes are restored
    # on teardown rather than leaking spec-less modules into other suites.
    parent_chain = [
        "vllm",
        "vllm.model_executor",
