...
def fake_block_table_module(monkeypatch):
    ...
    # Ensure parent packages exist. Inject via monkeypatch.setitem so they
    # are restored on teardown — a bare sys.modules[...] = ModuleType(...)
    # leaks a spec-less `vllm` module that makes a later
    # importlib.util.find_spec("vllm") raise "vllm.__spec__ is None" and
