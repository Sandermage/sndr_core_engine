...
class TestTextPatcherBasics:
    def test_applies_single_sub_patch(self, fake_source):
        ...
        content = Path(fake_source).read_text()
    ...
    def test_missing_required_anchor_is_skip(self, fake_source):
        ...
        before_content = Path(fake_source).read_text()
        ...
        after_content = Path(fake_source).read_text()
    ...
    def test_upstream_drift_marker_skips(self, fake_source):
        ...
        Path(path).write_text("# upstream_fix_ABC merged\n" + content)
        ...
        # No replacement done
        assert "return 1" in Path(path).read_text()
        assert "return 9" not in Path(path).read_text()
