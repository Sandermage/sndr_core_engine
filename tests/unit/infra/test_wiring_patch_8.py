...
def fake_p8_files(tmp_path, monkeypatch):
    ...
    from sndr.engines.vllm._archive import p8_kv_hybrid_reporting as p8
...
class TestPatch8Wiring:
    def test_apply_both_files(self, fake_p8_files):
        ...
        kv_content = Path(kv_path).read_text()
        sched_content = Path(sched_path).read_text()
    ...
    def test_idempotent(self, fake_p8_files):
        ...
        from sndr.engines.vllm._archive.p8_kv_hybrid_reporting import (
    ...
    def test_skip_when_upstream_merged_kv(self, fake_p8_files):
        ...
        content = Path(kv_path).read_text()
        Path(kv_path).write_text(
