...
class TestLiveCorpus:
    ...
    def test_strict_severity_returns_zero_on_clean_live_corpus(self):
        """--strict is GREEN on the live corpus (0 errors AND 0 warnings).

        The catalog is now fully clean: every builtin preset has a card,
        and the three production_candidate gemma4 presets that previously
        carried private-only evidence each gained a public smoke/tool-call
        evidence_ref (evidence/bench/gemma4_*/VERDICT_*.md). So the release
        `--strict` gate passes. If a future preset reintroduces a warning,
        this test fails — that is the intended release tripwire.
        """
        result = _run_cli("--strict")
        assert result.returncode == 0, (
            f"--strict should be green (0 err / 0 warn) on the live corpus; "
    ...
    def test_live_corpus_is_warning_free(self):
        """The live corpus carries ZERO warnings (release-clean state).

        Earlier in the V1 rollout the catalog tolerated two warning rules
        (`missing_card` for un-annotated presets, and
        `production_candidate_public_evidence` for prod-* presets whose
        evidence was private-only). Both classes are now resolved on the
        live corpus: every builtin preset has a card, and the three gemma4
        production_candidate presets each gained a public smoke/tool-call
        evidence_ref. The release `--strict` gate exits 1 on ANY warning,
        so the invariant here is simply: no warnings remain.

        Any warning reintroduced by a future preset edit fails this test
        (and would block the release gate) — investigate and either supply
        a public evidence_ref or annotate the missing card.
        """
        mod = _import_audit()
        report = mod.run_audit()
        warnings_list = [f for f in report.findings if f.severity == "warning"]
        assert warnings_list == [], (
            "live corpus must be warning-free for the release gate; got:\n"
            + "\n".join(
                f"  [{f.rule}] {f.preset_id}: {f.message}" for f in warnings_list
            )
        )
