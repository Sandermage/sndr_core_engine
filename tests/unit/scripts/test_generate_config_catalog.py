...
class TestRedactionEnforcement:
    ...
    def test_private_evidence_refs_redacted_with_marker(self):
        """Presets carrying private evidence get `{redacted: true, ...}`
        markers; co-present public repo-tracked refs are kept verbatim.

        `prod-gemma4-26b-multiconc` carries a mix: two private
        `external://sndr_private/...` refs (must be redacted) plus the
        public `evidence/bench/gemma4_26b_a4b/...` smoke + tool_call refs
        that clear the production_candidate_public_evidence gate (must be
        kept verbatim, never redacted, and must not leak a private path).
        """
        mod = _import_gen()
        rows = mod.build_catalog()
        target = next(
        ...
        redacted = [r for r in ev_refs if r.get("redacted") is True]
        public = [r for r in ev_refs if r.get("redacted") is not True]

        # The two private sndr_private refs must be redacted.
        assert redacted, "expected at least one redacted private evidence_ref"
        for ref in redacted:
            assert "sndr_private/" not in str(ref.get("path", "")), (
                f"redacted ref still leaks path: {ref}"
            )

        # The public evidence/bench refs must survive verbatim with a
        # repo-tracked (non-private) path.
        assert public, "expected the public evidence/bench refs kept verbatim"
        for ref in public:
            path = str(ref.get("path", ""))
            assert path.startswith("evidence/"), (
                f"public ref should be a repo-tracked evidence path: {ref}"
            )
            assert "sndr_private/" not in path, (
                f"public ref leaks a private path: {ref}"
            )
