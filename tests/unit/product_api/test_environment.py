...
def test_engine_installed_reflects_vllm_not_commercial_tier():
    ...
    # In a full dev environment vLLM is importable, so engine_installed must be
    # True there. On the dep-free release CI (no vLLM) the equality check above
    # already pins the behaviour; skip the absolute-True leg rather than fail.
    if not vllm_importable:
        pytest.skip("vllm not importable in this environment (dep-free CI)")
