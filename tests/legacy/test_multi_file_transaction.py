...
def test_all_files_apply_atomically(tmp_path):
    ...
    for p in (p1, p2, p3):
        assert p.marker in Path(p.target_file).read_text()
...
def test_dry_run_skip_when_anchor_missing_in_one_file(tmp_path):
    ...
    Path(p2.target_file).write_text("# no anchor here\n")
    ...
    assert "MARKER1" not in Path(p1.target_file).read_text(), (
...
def test_phase2_race_rollback_restores_files(tmp_path, monkeypatch):
    ...
    p1 = _make_synthetic_patcher(tmp_path, "f1.py", "AAA", "AAA_FIXED", "MARKER1")
    p2 = _make_synthetic_patcher(tmp_path, "f2.py", "BBB", "BBB_FIXED", "MARKER2")
    pre_f1 = Path(p1.target_file).read_text()
    ...
    post_f1 = Path(p1.target_file).read_text()
...
def test_phase2_race_rollback_skipped_does_not_unapply_idempotent(tmp_path):
    ...
    assert "PRIOR_MARKER" in Path(p_already.target_file).read_text()
    pre_fA = Path(p_already.target_file).read_text()
    ...
    pre_fF = Path(p_fail.target_file).read_text()
    ...
    assert Path(p_already.target_file).read_text() == pre_fA
    # NEW file (truly applied this txn) MUST be rolled back
    fN_post = Path(p_new.target_file).read_text()
...
def test_no_rollback_when_first_file_fails(tmp_path, monkeypatch):
    ...
    p2 = _make_synthetic_patcher(tmp_path, "f2.py", "BBB", "BBB_FIXED", "MARKER2")
    pre_f1 = Path(p1.target_file).read_text()
    pre_f2 = Path(p2.target_file).read_text()
    ...
    assert Path(p1.target_file).read_text() == pre_f1
    assert Path(p2.target_file).read_text() == pre_f2
