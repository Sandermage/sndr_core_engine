...
"""TDD tests for vllm.sndr_core.kernels.tq_decode_tune (Patch 18b).
...
class TestBlockKvOverride:
    ...
    def test_unset_returns_none(self, monkeypatch):
        from vllm.sndr_core.kernels.tq_decode_tune import get_block_kv_override
...
class TestNumWarpsOverride:
    ...
    def test_unset_returns_none(self, monkeypatch):
        from vllm.sndr_core.kernels.tq_decode_tune import get_num_warps_override
...
class TestNumStagesOverride:
    ...
    def test_unset_returns_none(self, monkeypatch):
        from vllm.sndr_core.kernels.tq_decode_tune import get_num_stages_override
...
class TestResolveDecodeTune:
    ...
    def test_all_unset_returns_upstream_defaults(self, monkeypatch):
        from vllm.sndr_core.kernels.tq_decode_tune import (
            resolve_decode_tune,
    ...
    def test_all_set_valid(self, monkeypatch):
        from vllm.sndr_core.kernels.tq_decode_tune import resolve_decode_tune
        monkeypatch.setenv("VLLM_TQ_DECODE_BLOCK_KV", "16")
...
class TestHasAnyOverride:
    ...
    def test_false_when_all_unset(self, monkeypatch):
        from vllm.sndr_core.kernels.tq_decode_tune import has_any_override
